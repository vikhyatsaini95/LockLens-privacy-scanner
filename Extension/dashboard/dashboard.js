document.addEventListener("DOMContentLoaded", async () => {
    ...
    const renderScan = async () => {
        ...
        else findings.forEach((finding) => { const card = document.createElement("article"); card.className = "finding-card"; const icon = document.createElement("span"); icon.className = "finding-icon"; icon.setAttribute("aria-hidden", "true"); icon.textContent = ({ name: "◉", email: "✉", phone: "⌁", address: "⌖", date: "◫", password: "◈" }[finding.type] || "◌"); const title = document.createElement("h3"); title.textContent = safeText(finding.label) || "Exposure category"; const count = document.createElement("span"); count.className = "finding-count"; count.textContent = `${Number.isFinite(finding.count) ? finding.count : 0} detected`; card.append(icon, title, count); box.append(card); });
